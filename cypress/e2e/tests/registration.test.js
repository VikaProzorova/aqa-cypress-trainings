...
describe('Registration form validation tests', () => {
  ...
  it('Validates Name field: length and invalid content', () => {
    ...
    cy.checkValidationErrorInvalidContent(sel.nameInput, invalidTooShort, name.invalidLength);
    cy.checkValidationErrorInvalidContent(sel.nameInput, invalidTooLong, name.invalidLength);
    cy.checkValidationErrorInvalidContent(sel.nameInput, invalidCyrillic, name.invalidData);
    cy.checkValidationErrorInvalidContent(sel.nameInput, invalidSymbols, name.invalidData);
    cy.checkValidationErrorInvalidContent(sel.nameInput, invalidNumbers, name.invalidData);
  });
  ...
  it('Validates Name field: known issues', () => {
    ...
    cy.checkValidationErrorValidContent(sel.nameInput, validName, { addSpaces: true });
  ...
  it('Validates Last name field: length and invalid content', () => {
    ...
    cy.checkValidationErrorInvalidContent(sel.lastNameInput, invalidTooShort, lastName.invalidLength);
    cy.checkValidationErrorInvalidContent(sel.lastNameInput, invalidTooLong, lastName.invalidLength);
    cy.checkValidationErrorInvalidContent(sel.lastNameInput, invalidCyrillic, lastName.invalidData);
    cy.checkValidationErrorInvalidContent(sel.lastNameInput, invalidSymbols, lastName.invalidData);
    cy.checkValidationErrorInvalidContent(sel.lastNameInput, invalidNumbers, lastName.invalidData);
  });
  ...
  it('Validates Last name field: known issues', () => {
    ...
    cy.checkValidationErrorValidContent(sel.lastNameInput, validLastName, { addSpaces: true });
  ...
  it('Validates Email field: invalid content', () => {
    ...
    invalidEmails.forEach(el => {
      cy.checkValidationErrorInvalidContent(sel.emailInput, el, email.invalidData);
    })
  });
  ...
  it('Validates Password field: invalid content', () => {
    ...
    invalidPasswords.forEach(el => {
      cy.checkValidationErrorInvalidContent(sel.passwordInput, el, password.invalidData, { isSensitive: true });
    });
  });

  it('Validates Password field: valid content', () => {
    cy.checkValidationErrorValidContent(sel.passwordInput, 'Qwerty12', { isSensitive: true });
  });

  it('Validates Re-enter password field: empty', () => {
    cy.get(sel.repeatPasswordInput).should('be.visible');
    cy.checkValidationErrorEmptyField(sel.repeatPasswordInput, repeatPassword.empty);
  });

  it('Validates Re-enter password field: passwords do not match', () => {
    cy.get(sel.passwordInput).clear().type('Qwerty12', { isSensitive: true });
    cy.checkValidationErrorInvalidContent(sel.repeatPasswordInput, 'Awerty12', repeatPassword.invalidMatch, { isSensitive: true });
  });
});
...
describe('Registration form Happy path', () => {
  ...
  it('Registration a new user', () => {
    ...
    cy.get(sel.passwordInput).clear().type(fakerUser.passwordData, { isSensitive: true });
    cy.get(sel.repeatPasswordInput).clear().type(fakerUser.repeatPasswordData, { isSensitive: true });

    cy.get(sel.submitButton).should('be.visible').click();
    cy.contains(selHeader.myProfileButton, 'My profile').should('be.visible').click();
